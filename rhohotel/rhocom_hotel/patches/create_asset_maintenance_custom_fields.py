...
def execute():
    # ── Remove required constraint from maintenance_team ──────────────────────
    frappe.db.set_value(
        "DocField",
        {"parent": "Asset Maintenance", "fieldname": "maintenance_team"},
        "reqd",
        0
    )

    # ── Custom fields for Asset Maintenance ───────────────────────────────────
    fields = [
        {
    ...
    for f in fields:
        if frappe.db.exists(
            "Custom Field",
            {"dt": "Asset Maintenance", "fieldname": f["fieldname"]}
        ):
            cf = frappe.get_doc(
                "Custom Field",
                {"dt": "Asset Maintenance", "fieldname": f["fieldname"]}
    ...
    for role in roles_needing_level1:
        exists = frappe.db.exists(
            "DocPerm",
            {"parent": "Asset Maintenance", "role": role, "permlevel": 1}
        )
        if not exists:
            frappe.get_doc({
                "doctype": "DocPerm",
                "parent": "Asset Maintenance",
                "parenttype": "DocType",
