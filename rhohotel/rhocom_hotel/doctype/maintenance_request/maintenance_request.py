...
#         # Create AND SUBMIT Asset Repair ONLY if it's a Repair request
#         if self.request_type == "Repair" and not self.asset_repair:
#             self.create_and_submit_asset_repair()  # New method that both creates and submits
...
#     def create_and_submit_asset_repair(self):
#         """Create and SUBMIT linked Asset Repair record immediately"""
#         clean_description = strip_html_tags(self.issue_description) if self.issue_description else ""
...
#         if hasattr(asset_repair_doc, 'submit') and asset_repair_doc.docstatus == 0:
#             asset_repair_doc.submit()
#             frappe.msgprint(
#                 f"Asset Repair <b>{asset_repair_doc.name}</b> has been created and submitted.",
#                 indicator="green",
#                 alert=True
#             )
