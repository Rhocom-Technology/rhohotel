// maintenance_task.js
// Place in: rhohotel/rhocom_hotel/doctype/maintenance_task/maintenance_task.js

frappe.ui.form.on('Maintenance Task', {

    setup(frm) {
        frm.set_query('assigned_to', () => ({ filters: { status: 'Active' } }));
        frm.set_query('supervisor', () => ({ filters: { status: 'Active' } }));

    },

    assigned_to(frm) { frm.trigger('validate_technician_supervisor'); },
    supervisor(frm) { frm.trigger('validate_technician_supervisor'); },

    validate_technician_supervisor(frm) {
        if (frm.doc.assigned_to && frm.doc.supervisor &&
            frm.doc.assigned_to === frm.doc.supervisor) {
            frappe.msgprint({
    ...
    end_time(frm) {
        if (frm.doc.start_time && frm.doc.end_time) {
            if (frappe.datetime.get_diff(frm.doc.end_time, frm.doc.start_time, 'seconds') <= 0) {
                frappe.msgprint({
