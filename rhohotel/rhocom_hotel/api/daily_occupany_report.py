...
def _get_room_status(room_status, housekeeping_status, checkin):
	if checkin:
		expected_checkout = checkin.get("expected_check_out_datetime")
		if expected_checkout and get_datetime(expected_checkout) < get_datetime():
...
def get_daily_occupancy_report(
	...
	date_to = _date_or_default(date_to, date_from)
	...
	for r in rooms:
		...
		row_status = _get_room_status(r.status, r.housekeeping_status, ci)
	...
	if floor:
		rows = [
			r for r in rows if str(r["floor"]) == str(floor) or str(r["room_number"]).startswith(str(floor))
		]
	...
	occupied_rooms = len([r for r in rows if r["status"] in ("Occupied", "Overdue Check-Out")])
	vacant_rooms = len([r for r in rows if r["status"] in ("Vacant Clean", "Vacant Dirty")])
	overdue_rooms = len([r for r in rows if r["status"] == "Overdue Check-Out"])
	...
	stats = {
		"occupancyRate": round((occupied_rooms / total_rooms) * 100, 1) if total_rooms else 0,
		"occupiedRooms": occupied_rooms,
		"vacantRooms": vacant_rooms,
		"arrivals": arrivals,
		"departures": departures,
		"outstanding": totals["outstanding"],
		"overdueCheckOut": overdue_rooms,
	}
