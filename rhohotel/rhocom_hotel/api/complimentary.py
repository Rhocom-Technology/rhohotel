...
CONSUMPTION_ROLES = {
    ...
    "Laundry": {"System Manager", "Hotel Manager", "Front Desk Manager", "Hotel Housekeeping", "Housekeeping Manager", "Sales User", "Sales Manager"},
...
def approve_complimentary(complimentary_name):
    ...
    For Late Checkout type: extends the checkout time and marks as Consumed.
    """
    try:
        doc = frappe.get_doc(DOCTYPE, complimentary_name)
        _require_write(doc)
        ...
        if doc.complimentary_type == "Room Upgrade":
            if not upgrade_room:
                return {"success": False, "error": _("Upgrade Room must be selected before approving a Room Upgrade voucher.")}
            if not doc.check_in:
                return {"success": False, "error": _("A linked Check-In is required to execute a Room Upgrade.")}
...
def _execute_room_upgrade(doc):
    ...
    try:
        ...
        frappe.db.rollback()
        return {"success": False, "error": _("Room transfer failed: {0}").format(str(e))}

    rate_invoice = result.get("rate_invoice")

    credit_note = None
    if rate_invoice:
        invoice_total = flt(frappe.db.get_value("Sales Invoice", rate_invoice, "grand_total"))
        if invoice_total > 0:
            try:
                ...
                frappe.log_error(
                    _("Rate invoice {0} was created but the waiver credit note failed: {1}").format(rate_invoice, str(e)),
...
def _execute_late_checkout(doc):
    ...
    frappe.db.set_value(
        "Hotel Room Check In",
        check_in_name,
        {"late_checkout": 1, "expected_check_out_datetime": new_checkout_dt},
