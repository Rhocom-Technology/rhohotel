...
#         if self.get("rh_approved") != "Approved":
#             frappe.throw(
#                 "This Asset Repair must be approved before submitting. "
#                 f"Current status: {self.get('rh_approved') or 'Pending'}",
#                 title="Approval Required"
#             )

#         if self.repair_status == "Completed" and not self.get("rh_vendor_technician"):
#             frappe.throw(
#                 "Please assign a technician before submitting a completed repair.",
#                 title="Technician Required"
#             )
...
#         """Every Asset Repair must be linked to a Facility Work Order."""
#         if not self.get("rh_facility_work_order"):
#             frappe.throw(
#                 "A Facility Work Order is required for every Asset Repair. "
#                 "Please link this repair to a Work Order.",
...
#         """
#         Pre-fill asset and technician from WO.
#         Auto-approve since WO approval covers this repair.
#         """
#         wo_name = self.get("rh_facility_work_order")
#         if not wo_name:
#             return

#         wo = frappe.get_doc("Facility Work Order", wo_name)

#         # Auto-approve — WO approval is sufficient
#         if self.get("rh_approved") in (None, "Pending", ""):
#             self.rh_approved = "Approved"
#             if not self.get("rh_approved_by"):
#                 self.rh_approved_by = frappe.session.user
#             if not self.get("rh_approved_on"):
#                 self.rh_approved_on = now_datetime()

#         # Asset — pre-fill from WO
#         if not self.asset and wo.asset:
#             self.asset = wo.asset

#         # Technician — pre-fill from WO assigned technician
#         if not self.get("rh_vendor_technician") and wo.assigned_technician:
#             self.rh_vendor_technician = wo.assigned_technician

#     # ── Approval permission ───────────────────────────────────────────────────

#     def _check_approval_permission(self):
#         """
#         Only Hotel Manager or System Manager can manually approve.
#         Bypassed when linked to a WO (auto-approved).
#         """
#         if (
#             self.has_value_changed("rh_approved")
#             and self.get("rh_approved") == "Approved"
#             and not self.get("rh_facility_work_order")
#         ):
#             allowed_roles = {"Hotel Manager", "System Manager"}
#             user_roles = set(frappe.get_roles(frappe.session.user))
#             if not allowed_roles.intersection(user_roles):
...
class CustomAssetRepair(AssetRepair):
    ...
    def before_submit(self):
        ...
        # Clear stock_items — stock reduction handled via Material Request → Stock Entry
        # Prevents double deduction of inventory
        self.stock_items = []
    ...
    def _validate_submit_fields(self):
        """
        Fields only required at submit time — not on draft save.
        Technician, Maintenance Representative, Functional Test Result,
        and Repair Status = Completed.
        """
        missing = []

        if not self.get("rh_vendor_technician"):
            missing.append("Technician")
        if not self.get("rh_maintenance_representative"):
            missing.append("Maintenance Representative")
        if not self.get("rh_functional_test_result"):
            missing.append("Functional Test Result")
        if self.repair_status != "Completed":
            missing.append("Repair Status (must be set to Completed)")

        if missing:
            frappe.throw(
                "The following fields must be filled before submitting:<br><br>"
    ...
    def _prefill_from_work_order(self):
        """
        Pre-fill asset and technician from WO.
        Auto-approve since WO approval already covers this repair.
        """
        wo_name = self.get("rh_facility_work_order")
        if not wo_name:
            return
    ...
    def _check_approval_permission(self):
        """
        Only Hotel Manager or System Manager can manually approve.
        Bypassed when linked to a WO since approval is auto-set.
        """
        if (
            self.has_value_changed("rh_approved")
            and self.get("rh_approved") == "Approved"
