      ...
      <div class="bg-white rounded-2xl w-full shadow-2xl overflow-y-auto" style="max-width:720px;max-height:92vh;">
        ...
        <div class="px-8 pt-8 pb-5 flex items-start justify-between border-b border-gray-100">
          <div>
            ...
            <p class="text-xs text-gray-400 mt-1">Collect payment and allocate against outstanding invoices for {{ checkIn.guest }}</p>
          </div>
        ...
        <div class="px-8 py-6 space-y-5">
          ...
          <template v-else>
            ...
            <div v-if="invoices.length === 0"
              ...
              <p class="text-xs text-green-500">All invoices for this check-in have been settled.</p>
            </div>
              ...
              <div>
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                  ...
                  <div>
                    ...
                    <input type="number" v-model.number="form.paid_amount" min="0.01" step="0.01"
                      class="w-full px-3 py-2.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                  </div>
                  <div>
...
const totalOutstanding = computed(() =>
  invoices.value.reduce((s, i) => s + (i.outstanding_amount || 0), 0)
)
...
onMounted(async () => {
  try {
    const [invData, mopData] = await Promise.all([
      apiPost('rhohotel.api.get_outstanding_invoices', { check_in: props.checkIn.name }),
      apiPost('frappe.client.get_list', { doctype: 'Mode of Payment', fields: JSON.stringify(['name']), limit: 50 }),
    ])
    invoices.value = invData.message || []
    paymentModes.value = mopData.message || []
...
async function submit() {
  ...
  error.value = ''
  try {
    // Distribute proportionally across outstanding invoices
    let remaining = form.paid_amount
    const allocations = []
    ...
    const data = await apiPost('rhohotel.rhocom_hotel.api.front_desk.collect_payment_for_checkin', {
    ...
    if (data.exc) { error.value = extractError(data); return }
